...
public class EntityProbe extends EntityProtossPassive {
	...
	protected void dropFewItems(boolean recentlyHit, int looting) {
		ItemDrop drop = new ItemDrop(50, new ItemStack(ItemHandler.PROTOSS_INGOT, 1 + this.rand.nextInt(2), EnumMetaItem.ProtossIngotType.KHALAI.getID()));
		drop.tryDrop(this);
	}
	...
	private void spawnElectricArc(double posX, double posY, double posZ) {
		for (int x = 0; x < 5; x++) {
			MinecraftHandler.getMinecraft().effectRenderer.addEffect(new EntityFXElectricArc(this.world, this.posX, this.posY, this.posZ, posX + this.rand.nextInt(2), posY, posZ + this.rand.nextInt(2), 10, 2.5F, 0.5F, 0.05F, 0xFF0000FF));
		}
	}
