...
import ocelot.api.client.gui.component.ComponentTextButton;
...
public class GuiPlayerMessage extends GuiBase {
	...
	private void reloadText() {
		...
		nextLine = 0;
		updateCount = 0;
		textLines = fontRendererObj.listFormattedStringToWidth(message.getText(), xSize / 2 - 16);
	...
	protected void renderGuiForegroundLayer(int mouseX, int mouseY) {
		for (int i = 0; i < currentText.size(); i++) {
			fontRendererObj.drawString(currentText.get(i), 0, 8 + 8 * i, 0);
		}
	}
