...
public class KorhalBiomeProvider extends BiomeProvider {
	...
	protected KorhalBiomeProvider() {
		super();
		biomeCache = new BiomeCache(this);
	}

	private KorhalBiomeProvider(long seed, WorldType worldTypeIn, String options) {
		this();
		GenLayer[] agenlayer = GenLayerKorhal.initializeAllBiomeGenerators(seed, worldTypeIn, options);
	...
	public Biome[] getBiomes(Biome[] listToReuse, int x, int z, int width, int length, boolean cacheFlag) {
		...
		if(cacheFlag && width == 16 && length == 16 && (x & 15) == 0 && (z & 15) == 0) {
			...
			for(int i = 0; i < width * length; ++i) {
				listToReuse[i] = Biome.getBiome(aint[i], BiomeHandler.biomeKorhalDesert);
			}
	...
	public Biome[] getBiomesForGeneration(Biome[] biomes, int x, int z, int width, int height) {
		...
		try {
			for(int i = 0; i < width * height; ++i) {
				biomes[i] = Biome.getBiome(aint[i], BiomeHandler.biomeKorhalDesert);
			}
