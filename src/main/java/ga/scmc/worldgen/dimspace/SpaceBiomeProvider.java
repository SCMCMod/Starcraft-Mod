...
public class SpaceBiomeProvider extends BiomeProvider {
	...
	protected SpaceBiomeProvider() {
		super();
		biomeCache = new BiomeCache(this);
	}

	private SpaceBiomeProvider(long seed, WorldType worldTypeIn, String options) {
		this();
		GenLayer[] agenlayer = GenLayerSpace.initializeAllBiomeGenerators(seed, worldTypeIn, options);
	...
	public SpaceBiomeProvider(WorldInfo info) {
	...
	public Biome[] getBiomes(Biome[] listToReuse, int x, int z, int width, int length, boolean cacheFlag) {
		...
		if (cacheFlag && width == 16 && length == 16 && (x & 15) == 0 && (z & 15) == 0) {
			...
			for (int i = 0; i < width * length; ++i) {
				listToReuse[i] = Biome.getBiome(aint[i], BiomeHandler.biomeSpace);
			}
	...
	public Biome[] getBiomesForGeneration(Biome[] biomes, int x, int z, int width, int height) {
		...
		try {
			for (int i = 0; i < width * height; ++i) {
				biomes[i] = Biome.getBiome(aint[i], BiomeHandler.biomeSpace);
			}
