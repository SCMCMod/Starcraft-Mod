...
public class KaldirBiomeProvider extends BiomeProvider {
	...
	protected KaldirBiomeProvider() {
		super();
		biomeCache = new BiomeCache(this);
	}

	private KaldirBiomeProvider(long seed, WorldType worldTypeIn, String options) {
		this();
		GenLayer[] agenlayer = GenLayerKaldir.initializeAllBiomeGenerators(seed, worldTypeIn, options);
	...
	public Biome[] getBiomes(Biome[] listToReuse, int x, int z, int width, int length, boolean cacheFlag) {
		...
		if(cacheFlag && width == 16 && length == 16 && (x & 15) == 0 && (z & 15) == 0) {
			...
			for(int i = 0; i < width * length; ++i) {
				listToReuse[i] = Biome.getBiome(aint[i], BiomeHandler.biomeKaldirIcePlains);
			}
