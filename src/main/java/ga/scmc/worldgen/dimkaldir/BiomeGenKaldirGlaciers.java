...
public class BiomeGenKaldirGlaciers extends BiomeHandler {

	public BiomeGenKaldirGlaciers(BiomeProperties id) {
		super(id);

		setRegistryName("kaldir_glaciers");
	...
	public final void genBiomeTerrainKorhal(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
		...
		for (int yLoc = 255; yLoc >= 0; --yLoc) {
			if (yLoc <= rand.nextInt(5)) {
				...
				if (origState.getMaterial() == Material.AIR) { // If we're still in the air...
					j = -1;
				} else if (origState.getBlock() == BlockHandler.FROZEN_STONE_KALDIR) { // If we've hit the ground...
					if (j == -1) { // If we were just in the air...
						if (randHeight <= 0) {
							topBlock = AIR;
							fillerBlock = BlockHandler.FROZEN_STONE_KALDIR.getDefaultState();
	...
	public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
		genBiomeTerrainKorhal(worldIn, rand, chunkPrimerIn, x, z, noiseVal);
	}
}