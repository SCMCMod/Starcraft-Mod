...
public class CharTerrainGenerator {
				...
				for (int height32 = 0; height32 < 32; ++height32) {
					...
					for (int h = 0; h < 8; ++h) {
						...
						for (int x = 0; x < 4; ++x) {
							...
							for (int z = 0; z < 4; ++z) {
								if ((d15 += d16) > 0.0D) {
									primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, BlockHandler.STONE_CHAR.getDefaultState());
								} else if (height < 63) {
									primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, Blocks.LAVA.getDefaultState());
								}
							}
