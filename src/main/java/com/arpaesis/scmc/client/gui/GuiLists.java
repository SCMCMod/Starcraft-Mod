...
import com.arpaesis.scmc.blocks.metablocks.BlockNeosteel.NeosteelMetalType;
import com.arpaesis.scmc.blocks.metablocks.BlockParisteel.ParisteelMetalType;
...
import com.arpaesis.scmc.handlers.WeaponHandler;
...
public class GuiLists
{
	...
	static
	{
		...
		tab1.add(new Product(new ItemStack(BlockHandler.GAS_COLLECTOR, 1, 1), 64, 16));
		TRADES[1] = tab1.build();

		ImmutableList.Builder<Product> tab2 = ImmutableList.builder();
		for (int i = 0; i < ParisteelMetalType.values().length; i++)
			tab2.add(new Product(new ItemStack(BlockHandler.PARISTEEL_METAL, 16, i), 10, 0));
		for (int i = 0; i < NeosteelMetalType.values().length; i++)
			tab2.add(new Product(new ItemStack(BlockHandler.NEOSTEEL_METAL, 16, i), 10, 0));
		...
		// LOGS.add(Utils.loadTextFromFile(new ResourceLocation(Starcraft.RL_BASE +
