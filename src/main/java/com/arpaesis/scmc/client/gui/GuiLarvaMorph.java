...
public class GuiLarvaMorph extends BasicGui {
	...
	public void openGUI(EntityPlayer player, Object mod, int guiID, World world, int x, int y, int z,
			EntityLarva larva) {
		...
		if (subOptions.size() <= 0) {
				...
				for (int x = 0; x < 5; x++) {
					...
					if (index < GuiLists.LARVA_OPTIONS.size()) {
						...
						if (GuiLists.LARVA_OPTIONS.get(index).getChildren().length <= 0) {
							tooltip.add(TextFormatting.GRAY + I18n.format("gui.larva_morph.tooltip.mineral_cost",
									GuiLists.LARVA_OPTIONS.get(index).getMineralCost()));
							tooltip.add(TextFormatting.GRAY + I18n.format("gui.larva_morph.tooltip.vespene_cost",
									GuiLists.LARVA_OPTIONS.get(index).getVespeneCost()));
						} else {
							tooltip.add(TextFormatting.GRAY + I18n.format("gui.larva_morph.tooltip.children",
									GuiLists.LARVA_OPTIONS.get(index).getChildren().length));
						}
			...
			for (int i = 0; i < subOptions.size(); i++) {
				...
				tooltip.add(TextFormatting.GRAY
						+ I18n.format("gui.larva_morph.tooltip.mineral_cost", option.getMineralCost()));
				tooltip.add(TextFormatting.GRAY
						+ I18n.format("gui.larva_morph.tooltip.mineral_cost", option.getVespeneCost()));
