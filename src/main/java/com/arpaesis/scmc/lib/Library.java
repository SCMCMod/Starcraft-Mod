	...
	public static void truncatedPyramid(World world, BlockPos pos, IBlockState state, int domeHeight, int domeTopLength, int slope)
	{
		...
		for (int x = -radius; x <= radius; x++)
			for (int z = -radius; z <= radius; z++)
				...
				if (world.getBlockState(pos.add(x, domeHeight - 1, z)).getBlock() == Blocks.SNOW_LAYER || world.getBlockState(pos.add(x, domeHeight - 1, z)).getBlock() == Blocks.AIR || world.getBlockState(pos.add(x, domeHeight - 1, z)).getBlock() == BlockHandler.PROTOSS_SHIELD)
