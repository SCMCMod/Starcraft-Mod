...
public class ChunkGeneratorKorhal implements IChunkGenerator
{
	...
	public ChunkGeneratorKorhal(World worldIn, long seed, boolean mapFeaturesEnabledIn, String generatorOptions)
	{
		{
	...
	public void populate(int x, int z)
	{
		...
		if (biome != BiomeHandler.biomeKorhalTerranCity && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes)
			if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, world, this.rand, x, z, false, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.LAVA))
			{
				...
				if (l2 < world.getSeaLevel() || this.rand.nextInt(this.settings.lavaLakeChance / 8) == 0)
				{
					// TODO: Make this tar once tar is added back in.
					(new KorhalGenCustomLakes(Blocks.WATER)).generate(world, this.rand, blockpos.add(i2, l2, k3));
				}
			}
