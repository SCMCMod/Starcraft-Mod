package com.arpaesis.api.world.entity;
...
public class Entities
{
	...
	public static RayTraceResult rayTraceSpecial(double reach, float partialTicks)
	{
		if (AccessHandler.getMinecraft().getRenderViewEntity() != null)
		{
			if (AccessHandler.getMinecraft().world != null)
			{
				...
				pointedEntity = null;
				Vec3d hitVec = null;
				List list = AccessHandler.getMinecraft().world.getEntitiesWithinAABBExcludingEntity(AccessHandler.getMinecraft().getRenderViewEntity(),
						AccessHandler.getMinecraft().getRenderViewEntity().getEntityBoundingBox().expand(lookVec.x * reach, lookVec.y * reach, lookVec.z * reach).expand((double) 1F, (double) 1F, (double) 1F));
	...
	{
		return rayTraceAll(entity.world, new Vec3d(entity.posX, entity.posY, entity.posZ), entity.rotationYaw, entity.rotationPitch, reach, new ArrayList<Entity>(Arrays.asList(new Entity[]
		{ entity })));
	}
	...
	public static Entity constructEntity(World world, Class<? extends Entity> c)
	{
		...
		try
		{
			return (c.getConstructor(World.class)).newInstance(new Object[]
			{ world });
		} catch (Exception e)
		{
	...
	{
		ArrayList<BlockPos> potentialLocations = com.arpaesis.api.world.block.Blocks.getCoordDataInRangeIncluding((int) around.getX(), (int) around.getY(), (int) around.getZ(), 2, toCheck.world, Blocks.AIR);
