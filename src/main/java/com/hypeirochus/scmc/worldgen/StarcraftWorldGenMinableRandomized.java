...
public class StarcraftWorldGenMinableRandomized extends WorldGenerator {
	...
	public StarcraftWorldGenMinableRandomized(IBlockState asteroidBase, IBlockState ore, int blockCount, int oreWeight, Predicate<IBlockState> p_i45631_3_) {
		this.asteroidBase = asteroidBase;
		this.ore = ore;
							...
							for (int j2 = l; j2 <= k1; ++j2) {
								...
								if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D) {
									...
									if (state.getBlock().isReplaceableOreGen(state, worldIn, blockpos, predicate)) {
										worldIn.setBlockState(blockpos, getRandomizedOreBlocks(asteroidBase, ore, oreWeight), 2);
									}
								}
							}
						}
