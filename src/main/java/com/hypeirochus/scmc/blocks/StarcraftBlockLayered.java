...
public abstract class StarcraftBlockLayered extends StarcraftBlock
{
	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos)
	{
		...
		return block != Blocks.ICE && block != Blocks.PACKED_ICE ? (iblockstate.getBlock().isLeaves(iblockstate, worldIn, pos.down()) ? true : (block == this && iblockstate.getValue(LAYERS).intValue() >= 7 ? true : iblockstate.isOpaqueCube() && iblockstate.getMaterial().blocksMovement())) : false; // FUCK
																																																																											// RIGHT
																																																																											// OFF,
																																																																											// VALIEC
	}
