...
public class EntityScarab extends EntityProtossMob implements IMob, Predicate<EntityLivingBase> {
	...
	/**
	 * Time when this scarab was last in an active state (Messed up code here, probably causes scarab animation to go weird)
	 */
	...
	public EntityScarab(World worldIn) {
		super(worldIn);
		setSize(.1F, .1F);
		this.setColor(EnumColors.WHITE);
		this.setFactions(EnumFactionTypes.AMONSFORCES);
	}
	...
	/**
	 * Params: (Float)Render tick. Returns the intensity of the scarab's flash when it is ignited.
	 */
