...
import com.hypeirochus.scmc.api.CapabilityUtils;
...
public class LivingEventHandler {
	...
	public static void onLivingUpdate(LivingUpdateEvent event) {
		if (event.getEntity() != null) {
			...
			if (event.getEntity().motionY > 0 && event.getEntity().dimension == ConfigHandler.INT_DIMENSION_CHAR) {
				event.getEntity().motionY = event.getEntity().motionY * 1.0375;

			}

			/** TODO: Handling Space gravity **/
			if (event.getEntity().dimension == ConfigHandler.INT_DIMENSION_SPACE) {
				...
				if (player.dimension != ConfigHandler.INT_DIMENSION_SPACE && player.posY > 255) {
					player.getServer().getPlayerList().transferPlayerToDimension(player, ConfigHandler.INT_DIMENSION_SPACE, new TeleporterHandler(player.world.provider.getDimension(), player.mcServer.worldServerForDimension(ConfigHandler.INT_DIMENSION_SPACE), player.posX, player.posY, player.posZ, true));
				}
