		...
		void processMessage(MessageGetFurnaceData msg, MessageContext ctx) {
			...
			if (tile instanceof TileEntityStarcraftFurnace) {
				...
				if (te.getCurrentItemBurnTime() != 0)
					NetworkHandler.INSTANCE.sendTo(new MessageReturnFurnaceData((float) te.getBurnTime() / (float) te.getCurrentItemBurnTime(), (float) te.getCurrentCookTime() / (float) te.getTotalCookTime(), msg.className, msg.burnTimeFieldName, msg.cookTimeFieldName), ctx.getServerHandler().player);
				else
