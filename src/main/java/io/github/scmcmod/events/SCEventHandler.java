package io.github.scmcmod.events;

import io.github.scmcmod.SCConstants;
import io.github.scmcmod.Starcraft;
import io.github.scmcmod.capabilities.ColorProvider;
...
import io.github.scmcmod.items.ItemGun;
...
public class SCEventHandler
{
	...
	public void onPlayerLoggedInEvent(PlayerLoggedInEvent event)
	{
		...
		String message = "Running SCMC version " + Starcraft.instance.getVersion() + "!";
	...
	public void cameraPosition(EntityViewRenderEvent.FOVModifier e)
	{
		...
		if (e.getEntity().dimension == SCConfig.INT_DIMENSION_SPACE)
		{
			if (e.getEntity().getRidingEntity() instanceof EntitySpaceship)
			{
				...
				{
					if (SCEventHandler.FOV < 120.0F)
					{
						e.setFOV(SCEventHandler.FOV += 0.1F);
					} else
					{
						SCEventHandler.FOV = 120.0F;
						e.setFOV(FOV);
					}
				} else
				{
					if (SCEventHandler.FOV > 90.0F)
					{
						e.setFOV(SCEventHandler.FOV -= 0.1F);
					} else
					{
						SCEventHandler.FOV = 90.0F;
						e.setFOV(FOV);
					}
				}
			}
