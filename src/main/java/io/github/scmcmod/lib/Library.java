...
public class Library
{
	...
	public static void square(World world, BlockPos center, int radius, IBlockState state)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			{
				if (world.getBlockState(pos).getBlock() == Blocks.AIR || world.getBlockState(pos).getBlock() == StarCraftBlocks.PROTOSS_SHIELD)
	...
	public static void truncatedPyramid(World world, BlockPos pos, IBlockState state, int domeHeight, int domeTopLength, int slope)
	{
		...
		for (int x = -radius; x <= radius; x++)
			for (int z = -radius; z <= radius; z++)
				...
				if (world.getBlockState(pos.add(x, domeHeight - 1, z)).getBlock() == Blocks.SNOW_LAYER || world.getBlockState(pos.add(x, domeHeight - 1, z)).getBlock() == Blocks.AIR || world.getBlockState(pos.add(x, domeHeight - 1, z)).getBlock() == StarCraftBlocks.PROTOSS_SHIELD)
