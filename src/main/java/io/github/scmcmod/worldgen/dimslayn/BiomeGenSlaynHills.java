...
public class BiomeGenSlaynHills extends BiomeHandler
{

	public BiomeGenSlaynHills(BiomeProperties id)
	{
		super(id);

		setRegistryName("slayn_hills");

		topBlock = StarCraftBlocks.DIRT_SLAYN.getDefaultState();
		fillerBlock = StarCraftBlocks.DIRT_SLAYN.getDefaultState();
		...
		for (int yLoc = 255; yLoc >= 0; --yLoc)
		{
			...
			} else
			{
				...
				{ // If we're still in the air...
					j = -1;
				} else if (origState.getBlock() == StarCraftBlocks.STONE_SLAYN)
				{ // If we've hit the ground...
					...
					{ // If we were just in the air...
						...
						{
							topBlock = AIR;
							fillerBlock = StarCraftBlocks.STONE_SLAYN.getDefaultState();
						...
						if (yLoc >= seaLevel - 1)
						{
							...
							chunkPrimerIn.setBlockState(xLoc, yLoc + 1, zLoc, StarCraftBlocks.DIRT_SLAYN.getDefaultState());
