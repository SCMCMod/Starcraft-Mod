package io.github.scmcmod.worldgen.dimslayn;
...
public class SlaynGenCustomLakes extends WorldGenerator
{

	private final Block block;

	public SlaynGenCustomLakes(Block blockIn)
	{
		block = blockIn;
	}
		...
		} else
		{
			...
			for (int k1 = 0; k1 < 16; ++k1)
			{
				for (int l2 = 0; l2 < 16; ++l2)
				{
					for (int k = 0; k < 8; ++k)
					{
						boolean flag = !aboolean[(k1 * 16 + l2) * 8 + k] && (k1 < 15 && aboolean[((k1 + 1) * 16 + l2) * 8 + k] || k1 > 0 && aboolean[((k1 - 1) * 16 + l2) * 8 + k] || l2 < 15 && aboolean[(k1 * 16 + l2 + 1) * 8 + k] || l2 > 0 && aboolean[(k1 * 16 + (l2 - 1)) * 8 + k]
				...
				for (int j3 = 0; j3 < 16; ++j3)
				{
					for (int j4 = 4; j4 < 8; ++j4)
					{
						if (aboolean[(i2 * 16 + j3) * 8 + j4])
						{
							...
							if (worldIn.getBlockState(blockpos).getBlock() == StarCraftBlocks.DIRT_SLAYN && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0)
							{
			...
			if (block.getDefaultState().getMaterial() == Material.LAVA)
			{
				for (int j2 = 0; j2 < 16; ++j2)
				{
					for (int k3 = 0; k3 < 16; ++k3)
					{
						...
						{
							boolean flag1 = !aboolean[(j2 * 16 + k3) * 8 + k4] && (j2 < 15 && aboolean[((j2 + 1) * 16 + k3) * 8 + k4] || j2 > 0 && aboolean[((j2 - 1) * 16 + k3) * 8 + k4] || k3 < 15 && aboolean[(j2 * 16 + k3 + 1) * 8 + k4] || k3 > 0 && aboolean[(j2 * 16 + (k3 - 1)) * 8 + k4]
							...
							{
								worldIn.setBlockState(position.add(j2, k4, k3), StarCraftBlocks.STONE_SLAYN.getDefaultState(), 2);
							}
						}
					}
				}
