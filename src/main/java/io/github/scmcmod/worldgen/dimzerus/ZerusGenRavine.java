...
public class ZerusGenRavine extends MapGenBase
{
	...
	private boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ)
	{
		...
		return (isExceptionBiome(biome) ? state.getBlock() == StarCraftBlocks.SAND_ZERUS : state.getBlock() == biome.topBlock);
	}
	...
	protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
	{
		net.minecraft.world.biome.Biome biome = world.getBiome(new BlockPos(x + chunkX * 16, 0, z + chunkZ * 16));
		IBlockState state = data.getBlockState(x, y, z);
		IBlockState top = isExceptionBiome(biome) ? StarCraftBlocks.SAND_ZERUS.getDefaultState() : biome.topBlock;
		IBlockState filler = isExceptionBiome(biome) ? StarCraftBlocks.STONE_ZERUS.getDefaultState() : biome.fillerBlock;

		if (state.getBlock() == StarCraftBlocks.STONE_ZERUS || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock())
		{
			if (y - 1 < 10)
			{
