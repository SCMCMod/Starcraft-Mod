...
public class ZerusGenCustomLakes extends WorldGenerator
{

	private final Block block;

	public ZerusGenCustomLakes(Block blockIn)
	{
		block = blockIn;
	}
				...
				for (int j3 = 0; j3 < 16; ++j3)
				{
					for (int j4 = 4; j4 < 8; ++j4)
					{
						if (aboolean[(i2 * 16 + j3) * 8 + j4])
						{
							...
							if (worldIn.getBlockState(blockpos).getBlock() == StarCraftBlocks.DIRT_ZERUS && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0)
							{
				...
				for (int j2 = 0; j2 < 16; ++j2)
				{
					for (int k3 = 0; k3 < 16; ++k3)
					{
						for (int k4 = 0; k4 < 8; ++k4)
						{
							...
							{
								worldIn.setBlockState(position.add(j2, k4, k3), StarCraftBlocks.STONE_ZERUS.getDefaultState(), 2);
							}
						}
					}
				}
