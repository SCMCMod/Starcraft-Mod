...
public class BiomeGenZerusMountains extends BiomeHandler
{

	public BiomeGenZerusMountains(BiomeProperties id)
	{
		super(id);

		setRegistryName("zerus_mountains");

		topBlock = StarCraftBlocks.GRASS_ZERUS.getDefaultState();
		fillerBlock = StarCraftBlocks.DIRT_ZERUS.getDefaultState();
		...
		for (int yLoc = 255; yLoc >= 0; --yLoc)
		{
			...
			} else
			{
				...
				{ // If we're still in the air...
					j = -1;
				} else if (origState.getBlock() == StarCraftBlocks.STONE_ZERUS)
				{ // If we've hit the ground...
					...
					{ // If we were just in the air...
						...
						{
							topBlock = AIR;
							fillerBlock = StarCraftBlocks.STONE_ZERUS.getDefaultState();
						...
						} else if (yLoc < seaLevel - 7 - randHeight)
						{
							topBlock = AIR;
							fillerBlock = StarCraftBlocks.STONE_ZERUS.getDefaultState();
							chunkPrimerIn.setBlockState(xLoc, yLoc, zLoc, StarCraftBlocks.GRAVEL_ZERUS.getDefaultState());
						} else
						{
