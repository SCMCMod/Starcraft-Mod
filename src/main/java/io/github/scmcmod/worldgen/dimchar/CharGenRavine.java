...
public class CharGenRavine extends MapGenBase
{
	...
	private boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ)
	{
		...
		return (isExceptionBiome(biome) ? state.getBlock() == StarCraftBlocks.DIRT_CHAR : state.getBlock() == biome.topBlock);
	}
	...
	protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
	{
		net.minecraft.world.biome.Biome biome = world.getBiome(new BlockPos(x + chunkX * 16, 0, z + chunkZ * 16));
		IBlockState state = data.getBlockState(x, y, z);
		IBlockState top = isExceptionBiome(biome) ? StarCraftBlocks.DIRT_CHAR.getDefaultState() : biome.topBlock;
		IBlockState filler = isExceptionBiome(biome) ? StarCraftBlocks.STONE_CHAR.getDefaultState() : biome.fillerBlock;

		if (state.getBlock() == StarCraftBlocks.STONE_CHAR || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock())
		{
			if (y - 1 < 10)
			{
