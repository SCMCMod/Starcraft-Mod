...
public class BiomeGenCharZergHive extends BiomeHandler
{

	public BiomeGenCharZergHive(BiomeProperties id)
	{
		super(id);

		setRegistryName("char_zerg_hive");

		topBlock = StarCraftBlocks.DIRT_CHAR.getDefaultState();
		fillerBlock = StarCraftBlocks.DIRT_CHAR.getDefaultState();
		...
		for (int yLoc = 255; yLoc >= 0; --yLoc)
		{
			...
			} else
			{
				...
				{ // If we're still
					// in the air...
					j = -1;
				} else if (origState.getBlock() == StarCraftBlocks.STONE_CHAR)
				{ // If
					// we've
					// hit
					// the
					...
					if (j == -1)
					{
						...
						{
							topBlock = AIR;
							fillerBlock = StarCraftBlocks.STONE_CHAR.getDefaultState();
						...
						if (yLoc >= seaLevel - 1)
						{
							...
							chunkPrimerIn.setBlockState(xLoc, yLoc + 1, zLoc, StarCraftBlocks.ZERG_CREEP.getDefaultState());
						} else if (yLoc < seaLevel - 7 - randHeight)
						{
							topBlock = AIR;
							fillerBlock = StarCraftBlocks.STONE_CHAR.getDefaultState();
							chunkPrimerIn.setBlockState(xLoc, yLoc, zLoc, StarCraftBlocks.GRAVEL_CHAR.getDefaultState());
						} else
						{
