package io.github.scmcmod.api.vector;
...
/* BlockVec3 is similar to Vector3
 * 
...
public class BlockVec3Dim implements Cloneable {
	...
	/**
	 * Get block ID at the BlockVec3Dim coordinates, with a forced chunk load if the coordinates are unloaded. Only works server-side.
	 *
	...
	private World getWorldForId(int dimensionID) {
		if (getEffectiveSide() == Side.SERVER) {
			return getWorldForDimensionServer(dimensionID);
		} else {
    ...
    private World getWorldForDimensionServer(int id)
    {
        MinecraftServer theServer = CoreUtil.getServer();
        if (theServer == null)
        {
