...
public class SCFurnace extends BlockContainer {
	...
	public void randomDisplayTick(IBlockState state, World worldIn, BlockPos pos, Random rand) {
		if (state.getValue(BURNING)) {
			...
			switch (enumfacing) {
			case WEST:
				worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, x - 0.52D, y, z + randOffset, 0.0D, 0.0D, 0.0D,
						new int[0]);
				worldIn.spawnParticle(EnumParticleTypes.FLAME, x - 0.52D, y, z + randOffset, 0.0D, 0.0D, 0.0D,
						new int[0]);
				break;
			case EAST:
				worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, x + 0.52D, y, z + randOffset, 0.0D, 0.0D, 0.0D,
				...
				break;
			case NORTH:
				worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, x + randOffset, y, z - 0.52D, 0.0D, 0.0D, 0.0D,
						new int[0]);
				worldIn.spawnParticle(EnumParticleTypes.FLAME, x + randOffset, y, z - 0.52D, 0.0D, 0.0D, 0.0D,
						new int[0]);
				break;
			case SOUTH:
				worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, x + randOffset, y, z + 0.52D, 0.0D, 0.0D, 0.0D,
	...
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos,
		...
		switch (this.type) {
		case CHAR:
			return new ItemStack(StarCraftBlocks.FURNACE_CHAR);
		case SHAKURAS:
			return new ItemStack(StarCraftBlocks.FURNACE_SHAKURAS);
		case SLAYN:
			return new ItemStack(StarCraftBlocks.FURNACE_SLAYN);
		default:
	...
	public int getComparatorInputOverride(IBlockState blockState, World worldIn, BlockPos pos) {
		return Utils.calculateRedstone(
				worldIn.getTileEntity(pos).getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null));
	}
