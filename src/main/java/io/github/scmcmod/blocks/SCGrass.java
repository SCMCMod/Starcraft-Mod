...
public class SCGrass extends BlockGeneric implements IGrowable {

	public static final PropertyBool SNOWY = PropertyBool.create("snowy");

	private IBlockState dirt;

	public SCGrass(String name, MapColor color, IBlockState dirt) {
			...
			if (worldIn.getLightFromNeighbors(pos.up()) < 4
				...
				if (worldIn.getLightFromNeighbors(pos.up()) >= 9) {
					for (int i = 0; i < 4; ++i) {
						...
						if (iblockstate1.getBlock() == dirt.getBlock()
	...
	public void grow(World world, Random rand, BlockPos pos, IBlockState state) {
		...
		for (int i = 0; i < 128; i++) {
			...
			while (true) {
				...
				placePos = placePos.add(rand.nextInt(3) - 1, (rand.nextInt(3) - 1) * rand.nextInt(3) / 2,
						rand.nextInt(3) - 1);

				if (world.getBlockState(placePos.down()).getBlock() != this
						|| world.getBlockState(placePos).isNormalCube()) {
					break;
				}
